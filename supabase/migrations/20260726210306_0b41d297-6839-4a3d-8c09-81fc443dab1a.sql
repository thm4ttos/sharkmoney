ALTER TABLE public.recurring_bills DROP CONSTRAINT IF EXISTS recurring_bills_payment_status_check;
ALTER TABLE public.recurring_bills ADD CONSTRAINT recurring_bills_payment_status_check
  CHECK (payment_status = ANY (ARRAY['pending'::text,'awaiting'::text,'partial'::text,'paid'::text,'late'::text]));
...
BEGIN
  ...
  -- Idempotência: clique duplo / retry não gera segundo pagamento.
