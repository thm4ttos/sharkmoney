...
AS $$
DECLARE
  caller uuid := NULLIF(current_setting('request.jwt.claim.sub', true), '')::uuid;
  removed jsonb := '{}'::jsonb;
  n int;
BEGIN
...
REVOKE ALL ON FUNCTION public.admin_reset_user_data(uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_reset_user_data(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_reset_user_data(uuid) TO service_role;