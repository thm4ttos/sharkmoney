...
-- 1) Allow authenticated/anon to evaluate has_role inside RLS policies
GRANT EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) TO authenticated, anon;

-- 2) Self-service reset: a user wipes ONLY their own financial/operational history
...
BEGIN
  ...
  DELETE FROM public.support_tickets       WHERE user_id = uid; GET DIAGNOSTICS n = ROW_COUNT; removed := removed || jsonb_build_object('support_tickets', n);
...
-- 3) Admin reset: an admin wipes any user's history
...
BEGIN
  ...
  DELETE FROM public.support_tickets       WHERE user_id = target; GET DIAGNOSTICS n = ROW_COUNT; removed := removed || jsonb_build_object('support_tickets', n);
